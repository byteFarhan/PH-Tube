...
const doSort = async (categoryId) => {
  ...
  // console.log(categoryDatas[0]?.others?.views);
  categoryDatas.forEach((data) => {
    const viewsArray = [];
    const views = parseInt(data.others.views);
    // console.log(views);
...
<p  class="absolute bottom-0 right-0 px-2 py-1 text-white rounded bg-title">${postedDate}</p>
      ...
      /* if (postedDate) {
        document.querySelector(".uplodeTime").innerText = `${postedDate}`;
      } else {
        document.querySelector(".uplodeTime").innerText = "";
      } */
      // console.log(data);
      //   const varified = data.authors[0].verified;
      //   const varified = data.authors[0].profile_name;
      //   console.log(varified);
      const varifieds = document.getElementsByClassName(".varified");
...
const convartSec = (sec) => {
  if (sec) {
    ...
    // document.getElementById(elementId).innerText = sentence;
    return sentence;
  }
  return "";
};
const mySec = convartSec(3720);
console.log(mySec);
hendleCategory();
